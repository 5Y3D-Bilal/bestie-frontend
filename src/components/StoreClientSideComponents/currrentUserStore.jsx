...
function CurrrentUserStore({ storeData }) {
  ...
  const UploadBanner = async () => {
    const res = await axios.put(
      `ttp://localhost:4000/api/store/updateStoreBanner/${storeData.map(
        (item) => item._id
      )}`,
  ...
  const UpdateStoreName = async () => {
    const res = await axios.put(
      `ttp://localhost:4000/api/store/updateStoreName/${storeData.map(
        (item) => item._id
      )}`,
  ...
  const UpdateStoreDescription = async () => {
    const res = await axios.put(
      `ttp://localhost:4000/api/store/updateStoreDescription/${storeData.map(
        (item) => item._id
      )}`,
  ...
  const UpdateStoreLocation = async () => {
    const res = await axios.put(
      `ttp://localhost:4000/api/store/updateStoreLocation/${storeData.map(
        (item) => item._id
      )}`,
